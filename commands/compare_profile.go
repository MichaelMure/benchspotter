...
func runCompareProfile(env *execenv.Env, options compareProfileOptions) error {
	...
	if options.baseSession == "" {
		baseInfo, err = inputs.SelectSession(env, env.Repo.GetRecall(baseRecallKey), profileFilter)
		if err != nil {
			return err
		}
	...
	if options.newSession == "" {
		newInfo, err = inputs.SelectSession(env, env.Repo.GetRecall(newRecallKey), profileFilter)
		if err != nil {
			return err
		}
